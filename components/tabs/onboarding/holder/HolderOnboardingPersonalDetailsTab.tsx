...
// Validations
import { HolderPersonalDetailsFormValidation } from "@/lib/validations/onboarding";
...
interface Props {
  handleFormSubmission: (data?: HolderFormInput) => Promise<void>;
}

export default function HolderOnboardingPersonalDetailsTab({
  ...
  const { handleSubmit, control, setValue, reset, clearErrors, watch } =
    useForm<HolderFormInput>({
      resolver: zodResolver(HolderPersonalDetailsFormValidation),
      defaultValues: {
        firstName: "",
        lastName: "",
        email: "",
  ...
  const onSubmit: SubmitHandler<HolderFormInput> = async (data) => {
    setLoading(true);
          ...
          <Controller
            ...
            render={({ field, fieldState: { error } }) => (
              <Textarea
                {...field}
                label="Bio"
                placeholder="Briefly describe your objective in using Maski"
                isInvalid={error ? true : false}
                errorMessage={error?.message}
                type="text"
                maxLength={500}
