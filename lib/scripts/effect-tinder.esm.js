import { likeEvent } from "../events/mobilePetSliderCustomEvents";
/**
...
export default function EffectTinder({ swiper, on }) {
  ...
  on("init", () => {
    console.log(swiper.tinder);
    if (swiper.el && swiper.el.addEventListener) {
      swiper.el.addEventListener("click", (e) => {
        if (e.target.closest(".swiper-tinder-button-yes")) {
          console.log("Yes clicked", swiper.tinder); // Check if the tinder object is available here
          swiper?.tinder?.yes?.bind(swiper.tinder)();
        }
        if (e.target.closest(".swiper-tinder-button-no")) {
          console.log("No clicked", swiper.tinder); // Check if the tinder object is available here
          swiper?.tinder?.no?.bind(swiper.tinder)();
        }
      });
    }
  });
  ...
  on("slideChange", () => {
    ...
    // console.log(swipeDirection < 0 ? "Bomboclaat" : "wakanda");

    // Only like the pet if the swipe direction is right and the slide has changed
    if (
      swipeDirection > 0 &&
      swiper.activeIndex !== touchStartIndex &&
      !isSlideChangeTouched
    ) {
      likeEvent.detail.likedPetIndex = swiper.activeIndex - 1;
